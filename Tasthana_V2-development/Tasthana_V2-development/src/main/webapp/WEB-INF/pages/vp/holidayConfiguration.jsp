...
<c:set var="baseURL" value="${fn:split(req,'/')}" />
		...
		function getDates(year,state,branchCode){
			...
			$('#mdp-demo').multiDatesPicker('removeDates', dates);
			 $.ajax({  
				    type: "GET",  
				    url: '<%=request.getContextPath()%>/vp/branchHolidayConfiguration?year='+parseInt(year)+'&state='+state+'&branchCode='+branchCode, 
...
<!-- 							<th>Edit</th> -->
	...
	// delete all rows except the first row from the table
	  ...
	  $.ajax({  
	    type: "GET",  
	    url: "<%=request.getContextPath()%>/vp/getState", 
	    contentType: "application/json",
	    dataType: "json",
	    data: dataString,
     async: false,
	    success: function(response){
	    	...
	    	for(i=0; i< response.length; i++){
		    	...
		    	trHTML += '<tr><td><input type="radio" onchange="getDatess();" class="myCheck" value="' + data.branchCode
				   ...
				   + '</td><td>' +data.area + '</td><td>' +data.cityOrTown + '</td><td>' +data.urbanOrRular + '</td></tr>'; 
