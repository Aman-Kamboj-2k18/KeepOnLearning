					...
					<c:if test="${! empty objList}">
								...
								<c:forEach items="${objList}" var="obj" varStatus="status">
									<tr>
										...
										<c:if test="${status.index+1 < objList.size()}">
											<td><a
												href="compareWithModification?modificationNo=${obj[3]}&preModificationNo=${objList[status.index+1][3]}"><button>Show
														Modification Report</button></a></td>
										</c:if>

										<c:if test="${status.index+1 == objList.size()}">
											<td><a
												href="compareWithDeposit?modificationNo=${obj[3]}&depositId=${obj[0]}"><button>Show
														Modification Report</button> </a></td>
										</c:if>
                     ...
                     <table align="center" class="f_deposit_btn">
							<tr>
								 ...
								 <td><a href="modifiedDeposits" class="btn btn-danger"><spring:message
