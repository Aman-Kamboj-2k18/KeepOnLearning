...
.ready(
		function() {
			var effectiveDateFormatted = '<fmt:formatDate type="date" pattern="dd/MM/yyyy" value="${addRateForm.effectiveDate}"/>';
			debugger
			 citizenFun();
         ...
         $("#effectiveDate").val(effectiveDateFormatted);
...
function populateRate(){
	...
	$.ajax({  
		async: false,
		type: "GET",  
	    url: "<%=request.getContextPath()%>/common/getRateDurationsByDepositClaasification", 
...
function submitData(){
	 ...
	 var taskArray = new Array();
		...
		var dataString = 'category='+ category+ '&effectiveDate=' + effectiveDate + '&currency=' + currency + '&amountSlabFrom=' + amountSlabFrom + '&amountSlabTo=' + amountSlabTo + '&depositClassification=' + depositClassification + '&citizen=' + citizen + '&nriAccountType='+nriAccountType+ '&menuId='+ ${menuId};;
		debugger;
		var url="<%=request.getContextPath()%>/common/addRatePost";
		...
		$.ajax({
			  url: url,
						type : 'post',
						async : true,
						data : "rateArrList=" + taskArray + '&dataString='+ dataString,
...
function categoryAndDepositOrAccount() {
	...
	function getDepositClassification(){
		...
		var depositClassification= document.getElementById('depositClassification').value;
		 ...
		 var  menuId = ${menuId};
		$("#addRateForm").attr("action", "getRateDurationsByDepositClaasification?depositClassification="+depositClassification+"&category="+category+"&nriAccType="+nriAccType+"&currency_="+currency_+"&menuId=" +menuId);
		$("#addRateForm").submit();
