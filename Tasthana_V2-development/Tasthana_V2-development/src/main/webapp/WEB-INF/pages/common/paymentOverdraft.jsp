...
function myFunction() {
	...
	data='';
	  ...
	  $.ajax({  
	    type: "GET",  
	    url: "<%=request.getContextPath()%>/common/getHolderDetails", 
...
function showDetails(obj) {
	...
	if (paymentMode == 'Debit from Linked account') {
		document.getElementById('bankDetailsTr').style.display = 'none';
		document.getElementById('cardDetailsTr').style.display = 'none';
		document.getElementById('netBankingDetailsTr').style.display = 'none';
	...
	//alert(draft);
	 var dataString = 'overdraftNumber=' + draft;
		data='';
		  ...
		  $.ajax({  
		    type: "GET",  
		    url: "<%=request.getContextPath()%>/common/getOverdraftIssueDetails", 
		    contentType: "application/json",
		    dataType: "json",
		    data: dataString,
	        async: false,
		    success: function(response){ 
		    	...
		    	if(data.amountToReturn!=null && data.amountToReturn!=undefined)
