...
<c:set var="baseURL" value="${fn:split(req,'/')}" />
		...
		function d(year){
			...
			$('#mdp-demo').multiDatesPicker('removeDates', dates);
			 $.ajax({  
				    type: "GET",  
				    url: '<%=request.getContextPath()%>/common/holidayConfigurationGetByYear?year='+parseInt(year), 
		...
		function selectYear(obj) {
			...
			$('#mdp-demo').datepicker('option', 'minDate', new Date(year,0,1));
			$('#mdp-demo').datepicker('option','maxDate', new Date(year,11,31)); */
	...
	<div class="container-fluid">
		...
		<div class="Flexi_deposit">
			...
			<div class="header_customer">
				<h3>StateWise Holiday Configuration</h3>
			</div>
			...
			<form:form  commandName="holidayConfiguration" action = "stateHolidayConfiguration" method="post" id="holidayConfiguration" class="form-horizontal" autocomplete="off">
					...
					<div class="form-group" id="states">
							...
							<form:select id="state" path="state"
								class="input myform-control" onChange="getDatess()">
					...
					<%-- <tbody>
						...
						<label class="col-md-3 control-label" style="padding-top: 16px;">Year<span
								...
								<select onchange= "selectYear(this)" name="year" id="year" class="myform-control">
...
function getDates(year,state){
	...
	$('#mdp-demo').multiDatesPicker('removeDates', dates);
	 $.ajax({  
		    type: "GET",  
		    url: '<%=request.getContextPath()%>/common/holidayConfigurationGetByYear?year='+parseInt(year)+'&state='+state, 
		    contentType: "application/json",
		    dataType: "json",
		    async:true,
		    success: function(response){ 
